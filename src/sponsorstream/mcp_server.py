"""Legacy MCP server for read-only campaign matching.
...
mcp = FastMCP("sponsorstream-legacy")


@mcp.tool()
def campaigns_match(query: str, top_k: int = 10) -> str:
    """Match campaigns by text query. Returns similar creatives based on semantic similarity.

    Args:
        query: Text query to match against campaigns
        top_k: Number of results to return (default: 10, max: 100)

    Returns:
        JSON string containing list of matching creatives with scores and metadata
    """
